...
description: Dialup フレームワークの新バージョン公開準備。mix docs と dialup-site の整合確認、検証、mix hex.build / dry-run まで行い、最終の mix hex.publish はユーザーが手動で行う。ユーザーが新バージョン公開・バージョンアップを依頼したときに使う。
...
- 対象パッケージ: `:dialup`（**リポジトリルート**の `mix.exs`）。`site/` の `:dialup_site` は Hex 公開対象外
- 公開先: Hex（`mix hex.publish` は **ユーザーが手動実行**）
...
### 5. パッケージ内容確認（エージェントの最終ステップ）

**必ずリポジトリルートで実行する。** `cd site` した状態で `mix hex.publish` すると `:dialup_site` が対象になり、`description` / `licenses` / `links` 不足や path 依存の `dialup` 除外で失敗する。
...
- dry-run が通ったら **ここでエージェントは停止** し、ユーザーに手動公開を案内する

### 6. 公開（ユーザー手動）

エージェントは `mix hex.publish` を実行しない。ユーザーがルートで:
...
- `key not authorized for this action` が出たら、`:dialup` の maintainer 権限を持つ Hex API key / アカウントで再認証する
- 公開後、`mix hex.info dialup` でバージョンが反映されたか確認する
...
7. 公開前なら、ユーザー向けにルートでの `mix hex.publish` 手順を短く案内する
